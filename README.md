...
| `reverse` | reconstructing how an achieved result can be replicated | Deliver working implementation assets and a ready agent brief with exact adaptation/checks; retain causal drilldown and separate adoption |
...
Use `/atlas:reverse <result, optimization ledger or PR>` (Cursor adapter: `/atlas-reverse`) to deliver an operational reuse kit: actual source assets, exact entrypoints, adaptation seams, invariants, focused checks and a ready agent brief. Its causal drilldown traces result -> intermediate changes -> mechanisms -> actual practices -> enabling conditions. Trace every material branch within the declared evidence to a reproducible action or precise gap, then check the method forward. Render nontrivial traces with drilldown into evidence. Each supported practice receives a point of use, conditional default and verified activation or explicit pending state; finding one win or saving a note does not complete the set. Preserve necessary bundles and uncertain attribution. `--read-only` returns proposed integrations without writes.
