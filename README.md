...
| `reverse` | reconstructing how an achieved result can be replicated | Recover evidenced patterns and transferable principles; expand causal evidence with --drilldown or implementations with --kit |
...
| `ship` | committing verified work, pushing changes, opening a PR or preparing its description | Deliver a concise dossier by default: drilldown, cross-domain impact, risks and declared code owners; explicit opt-out allowed |
...
Use `/atlas:reverse <result or PR>` (Cursor: `/atlas-reverse`) for recursive causal drilldown followed by named patterns and compact operating rules transferable beyond the original domain. Compare distinct instances, failures and counterexamples; repeated accounts count once. Recurrence, causal support and validated transfer stay distinct; a single-case pattern remains a labelled candidate. Depth follows the evidence and unresolved causal questions; output length follows useful meaning, with no forced single line. Every expanded result ends in that distillation: `--drilldown` opens mechanisms, evidence and limits; `--kit` opens actual implementations and adaptation checks. Each principle retains its causal basis and is challenged in another domain; decisive qualifiers remain visible. Learning integrates the rules at their authorized point of use and distinguishes case success, documented knowledge, activation and actual reuse. Read-only returns the directive without writes.
...
Optimize checkpoints every verified gain, including useful alternatives: local commit, retained lightweight ref and ledger read-back before promotion or another round. Normal cleanup removes disposable worktrees, preserving checkpoint history. Every ship delivery includes a concise revision-bound dossier with changed-path drilldown, cross-domain effects, material risks and CODEOWNERS when present. --no-dossier or an explicit summary-only request overrides presentation. Local delivery keeps the dossier inline/local; only authorized PR delivery publishes it. Shared rationale, owners and checks appear once; details expand only when useful.
...
- `hooks/claude-codex.json` — Claude Code and Codex. `SessionStart` and
...
- Hooks: same `hooks/claude-codex.json`; trust it once in `/hooks` (see Install).
...
- Codex substitutes `${CLAUDE_PLUGIN_ROOT}` in `hooks/claude-codex.json` only. Inside a skill body neither host expands a variable, and Codex presents skills to the model as absolute skill roots, so every path in a skill is written relative to the file that names it (`<this skill>/scripts/…`, `<plugin root>/scripts/…`); `scripts/check.sh` fails on any `CLAUDE_SKILL_DIR` or `${CLAUDE_PLUGIN_ROOT}` inside a skill or agent.
