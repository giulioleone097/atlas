...
Small changes use implementation, review/fix and proof; broader work uses scope -> build -> review and authorized ship. Optimize checkpoints verified gains in Git. Ship defaults to a concise dossier: drilldown, cross-domain impact, risks and declared code owners. atlasme/howto share the decision engine. Build’s references/tickets.md owns tracker authority/lifecycle; report verified owners. Questions and assessments do not authorize edits.
...
Stop at proven acceptance, including efficiency. Close intentions as done, blocked or dropped with reason. Report outcome, proof, risks and next action concisely; preserve meaning, never code golf. Every workflow reuses applicable practices and drills into new verified results through ship's learn step: trace -> validate -> integrate -> apply -> measure. Account for every supported practice at its point of use; a note is not integration. Reuse valid evidence; resume pending integrations only after their blocker changes. Preserve scope, uncertainty, authority and activation proof. Measure compound benefit; never sum isolated gains.
<!-- atlas:core:end -->
