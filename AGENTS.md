...
For small understood changes: state outcome/proof, implement, review/fix, verify. Broader work runs scope -> build -> review; ship runs when authorized. howto owns brainstorming through reviewed scenarios, authorized execution and evidence-based adaptation; atlasme explores an idea and hands that same decision evidence to howto when work is authorized. Both use skills/scope/references/atlasme.md and its scenarios.md, not competing plans or councils. Settled work uses scope directly. Other named entry points retain their contracts; questions and assessments do not authorize edits.
