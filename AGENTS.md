...
For work, read the native lifecycle reference via the installed Atlas build skill or its exported procedure, at entry, resume and final proof. Preserve the enclosing goal.

Read references/context.md through the installed Atlas scope skill or its exported procedure before context-dependent decisions: relevant declared wiki pages, project facts and existing rationale, not the whole knowledge base. Reuse current receipts; stale or missing evidence stays explicit. Use that same skill's references/composition.md for bounded compatible specialists discovered on the live host; retain one owner and return proof without controller cycles. Atlas works independently of other plugins.

Small understood changes use direct implementation, review/fix and proof. Broader work runs scope -> build -> review; ship runs when authorized. atlasme explores an idea; howto carries its same evidence through reviewed scenarios, optional evolution levels, authorized work and actual proof. Both reuse scope's decision engine. Other entry points retain their contracts. Questions and assessments do not authorize edits.
...
Delegation: use economical subagents for independent work; the lead coordinates scope, decisions and integration. Agents declare `sonnet`, `opus` or `haiku`, resolved by host through `agents/models.json` (Codex `gpt-6-astra`/`gpt-5.6-sol`/`gpt-5.6-terra`/`gpt-5.6-luna`; Cursor `inherit`; Devin session model). Contracts name outcome, paths, exclusions and proof; workers preserve edits, the lead verifies reports and owns repairs. Keep trivial work inline; honor model/effort pins or report them unavailable. Material scenarios auto-evaluate interaction; `--simulate` requests it without inventing facts/actors. The caller alone coordinates native read-only actors: supplied state, no tools/actions/votes, exact barriers/revisions, root-shared child budget, recursive reconciliation, explicit skipped/evidence/unavailable results.
