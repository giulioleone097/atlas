...
4. Read reviewer evidence alongside the code. Take the merged PRs in the window and read relevant inline comments/reviews through the native forge. Repeated requests identify convention candidates, not proof or authority; verify them against final changes, repository constraints and checks through learn's evidence gate. Group supported themes with their source PR/revision and exceptions. Do not infer a rule from reviewer popularity or treat absent comments as approval.
