...
2. Bind or reuse the native lifecycle through `references/native.md` before work, following its authorization and parent-goal rules. Route. Preserve inherited ticket intent: `Tickets: requested` or an explicit request to create tickets is `--tickets`. With `--tickets`, read and perform `references/plan.md` before implementing, even for a small change. Otherwise a small understood change follows core's short path; finish it here, including local review and fixes, without another skill handoff. A failure with no known cause: read `<this skill>/references/debug.md` (`<this skill>` is the directory this file lives in). `Size: complex`, several owners, a change others depend on, or `--tickets` (publishes the plan's tasks; reuse a plan already read in this step): read `references/plan.md`. Load `references/fix.md`, `references/refactor.md` or `references/migrate.md` only for the applicable work, and `references/ui-taste.md` only when making visual design decisions.
...
Then invoke `review` on the actual task diff (Skill tool `atlas:review`, `$review` on Codex) unless the short path already completed local review or `--no-review` was given; the flag skips only that handoff, and the short path's local review still runs. After review, reconcile native completion through `references/native.md` only at the outer workflow boundary; requested shipping remains part of the enclosing acceptance. Do not commit here; that is ship's step, and it runs when asked.
