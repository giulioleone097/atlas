...
Read `<plugin root>/core/ATLAS.md` once if not loaded; `<plugin root>` is the parent of this skill's `skills/` directory. Carry scope, authorization and interactive/unattended context throughout. `--decision-only` or a map-only request returns the map; `--card-only` produces a scope card without implementation or publication. `--tickets` retains publication intent for an authorized build route; it is not implementation permission. `--simulate` requests the bounded native actor procedure; carry it through the shared engine reopening only the affected node when newly requested. A saved map and its `next:` line are context, not fresh authority. Propagate explicit read-only intent into children, including vocabulary/context handling; it permits no file writes or work handoffs.
...
3. Resolve the requested evolution mode (none/all/next/leap/transform) against the saved mode and reviewed inputs. A new mode, newly requested `--simulate`, or an explicit request for new alternatives reopens only the affected approach node; if none exists, add one ready node even when the destination/map was settled. Preserve the user's goal. After review, record the mode and consequential input revisions so the same unchanged request reuses it instead of reopening on every loop. Read `<plugin root>/skills/scope/references/atlasme.md`, the single shared brainstorming/decision engine. Retain `--simulate`, `--evolve [next|leap|transform]` or equivalent intent as an optional input, never implementation permission. It composes scenario exploration and decision review from `<plugin root>/skills/scope/references/scenarios.md`; do not build another comparison here or call public atlasme back. Reuse a reviewed tree supplied by atlasme, existing stable decision/prerequisite IDs and material rejected alternatives. A simple settled subject needs no artificial nodes. Keep actions as evidence checkpoints with inputs, smallest useful step, observation and known time/cost budget or `unknown`. Forecasts are not completed work. Link the existing issue/plan; do not create a second backlog or one ticket per node.
...
simulation: <not run | capability mode; source/input revisions; rules; branch state; branch-keyed actor memory/identities; accepted/rejected and uncommitted exact-revision proposals; pending round; next action>
