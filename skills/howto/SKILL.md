---
name: howto
description: Use when an objective needs a path through dependent decisions. Establish the outcome and proof, resolve ready decisions, retain evidence and checkpoints, and resume or hand off authorized work.
...
3. Read `<plugin root>/skills/scope/references/atlasme.md` for the decision engine. Keep one node per material decision, with stable IDs and prerequisite IDs. A simple settled subject needs no artificial nodes. Separate decisions from actions: outcome checkpoints name the input, smallest useful step and observation that will verify it; they are planned work, not completion claims. Future learning that does not block the current choice stays a checkpoint or sourced assumption, not an invented question. Keep existing issue references; this map does not own ticket status or publish one issue per node.
...
5. Invoke `atlasme --decision-only` on the ready decision batch with its prerequisites and constraints (public `<plugin root>/skills/atlasme/SKILL.md`, following the core skill execution contract). Its shared asking contract owns interactive rounds. In unattended work, do not invoke a question round: record missing user choices as waiting, continue independent facts/checkpoints, then go to step 6 when no work can progress. Retain each actual choice, material rejected alternative/reason, source and relevant reach/risk. Merge sharpened decisions, recompute the frontier and continue while progress is possible; there is no per-session node quota. A pending required reply remains pending. An explicit pause stops the requested scope; a deferred choice or blocker stops only its dependent work. A child's return alone does not end this route.
...
## Checkpoints
- C1 needs: <decision/checkpoint IDs or source> -> <smallest useful step> -> <observable proof>; result: <planned | verified evidence | blocked reason>; owner: <existing issue/plan | unassigned>
...
7. Return the map when only the map was requested or decisions remain deliberately open/blocked; required interactive replies still follow the asking lifecycle. When clear and a card was requested, invoke `scope --card-only`; when implementation was already requested, invoke `scope` (public `<plugin root>/skills/scope/SKILL.md`, following the core skill execution contract). Pass the saved map or unsaved in-turn content and `--tickets` when requested; a card retains that intent without publishing. Scope owns acceptance and build owns the execution plan/tickets. Run the authorized handoff now, return its verified result and persistence gaps, and never report the destination achieved merely because its decision map is clear.
