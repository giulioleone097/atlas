...
3. Read `references/context.md` when durable context can change the outcome, reusing a current receipt. Resolve technical options through core's Intent rule; reuse current selected options and evidence instead of asking for missing flags. The decision engine owns brainstorming and composes `references/scenarios.md` for scenario comparison, bounded native simulation when requested/material, and decision review; reuse its findings. If it returns `checkpoint-needed`, hand the same subject and evidence to public `<plugin root>/skills/howto/SKILL.md` with current authority, selected simulation/evolution/experiment options, remaining bounds and explicit card/ticket constraints, then return that result. Do not scope an unsettled whole outcome or use review as implementation permission. Otherwise find the remaining facts in the identified code, relevant `docs/atlas/map.md` and settled `CONTEXT.md`; never ask for facts available there.
