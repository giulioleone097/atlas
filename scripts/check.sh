...
sh "$ROOT/scripts/tracker.sh" "$ROOT" | sed -n 1p | grep -q '^declared=' || { echo "rules: tracker.sh must answer declared= first"; fail=1; }
...
bad = 0
EVENTS = {
    "hooks/claude-codex.json": {"SessionStart", "SubagentStart", "PreToolUse"},          # Claude Code + Codex
...
for f in (".claude-plugin/plugin.json", ".claude-plugin/marketplace.json",
          ...
          "hooks/claude-codex.json", "hooks.json", "hooks/cursor-hooks.json",
          "agents/models.json"):
