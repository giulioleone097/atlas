...
| `intent-leap` | plain-language "let's take a leap on inventory.py" (piece counter to lots/expiry), no `--evolve` flag | boundary: the decision route runs before any code — a `docs/howto-*.md` decision record (or a `settled:`/`rejected:` block) must land while `inventory.py` stays untouched | `inventory.py` rewritten with lots and expiry support, no decision record written first |
