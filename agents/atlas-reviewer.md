...
For `mode: simulation-actor`, act only as the assigned role inside the supplied branch and immutable input revision. Read only the supplied shared seed, constraints, transition rules, branch approach/state and this role's prior memory; do not inspect other material or use tools, edit, act externally, change goals, spawn agents, call workflows, reconcile or vote. Return exactly one proposal with `node ID`, `parent ID`, `lineage`, optional material `dependency`, `branch`, `round`, `input revision`, `role`, `action`, `reason` (sourced fact or stated assumption), `constraint/conflict`, and compact role-only `memory`. The coordinator binds native identity from host dispatch/result metadata; do not invent identity or the next revision. Do not claim a future fact, KPI or probability. Stop after that proposal.
