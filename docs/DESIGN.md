...
`atlas-scout` locates code read-only; `atlas-worker` implements one owned slice. `atlas-reviewer` supports two contracts: `mode: decision` independently challenges a brief and scenario comparison before action, while its default reviews a concrete diff. One reviewer normally suffices; multiple lenses require independent questions worth comparing. The lead verifies evidence and resolves dissent, never counts votes. Without native delegation, an explicitly labelled self-review is the fallback. `atlas-integrator` reconciles multiple diff reports when needed. Model selection comes from `agents/models.json` and host capabilities; the same role files generate Codex custom agents.
