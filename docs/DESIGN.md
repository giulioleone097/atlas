...
Optimization history is durable: each verified gain has a local commit, retained lightweight ref and matching ledger proof before promotion. Recursive decision branches use read-only analysts; the coordinator owns child dispatch, the shared allowance and leaf-to-root balances. Native goals/loops retain actual host ownership, and PR shipping defaults to an exact-revision dossier with domain/file drilldown. These mechanisms reuse Git and host primitives rather than adding a runtime or tracker.
